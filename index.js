...
client.once('ready', () => {
  ...
  console.info(`Empezando a iniciar el bot.`)
  for (const file of commands) {
    const command = require(`./commands/${file}`);
    client.commands.set(command.name, command);
    console.log(`> Cargando '${config.prefix}${command.name}'`);
  }
  ...
  if (client.guilds.cache.get(config.guildID).member(client.user).hasPermission("ADMINISTRATOR", false)) {
    ...
    setTimeout(function() {
      console.log(`Required permissions have been granted\n\n`)
    }, 1250);
    ...
    console.error(`Required permissions have not been granted`)
    console.error(`Please give the bot the 'ADMINISTRATOR' permission\n\n`)
    if (config.useEmbeds) {
      const embed = new Discord.MessageEmbed()
        .setAuthor(`${client.user.username} / Ticket Log`, client.user.avatarURL)
        .setColor("#E74C3C")
  ...
  if (config.useEmbeds) {
    const embed = new Discord.MessageEmbed()
      ...
      .addField("**Rewards**", `Cooming Soon\n`)
      .setFooter("Axuze Community")
      .setTimestamp()
...
client.on('message', async message => {
  ...
  if (message.channel.type === "dm") {
    ...
    if (config.logDMs) {
      if (config.useEmbeds) {
        const embed = new Discord.MessageEmbed()
          .setAuthor(`${client.user.username} / Ticket Log`, client.user.displayAvatarURL())
          .setTitle("DM Logger")
  ...
  try {
    ...
    console.error(`An unknown error occured whilst executing the '${command.name}' command`);
  }
...
client.on('error', error => {
  console.warn(`Potential error detected\n(likely Discord API connection issue)\n`);
  console.error(`Client error:\n${error}`);
});
client.on('warn', (e) => console.warn(`${e}`));

if(config.debugLevel == 1){ client.on('debug', (e) => console.debug(`${e}`)) };

process.on('unhandledRejection', error => {
  console.warn(`An error was not caught`);
  console.error(`Uncaught error: \n${error.stack}`);
});
process.on('beforeExit', (code) => {
  console.log(`Disconected from Discord API`);
